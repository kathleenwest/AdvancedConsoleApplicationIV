...
{
    /// <summary>
    /// Using Threads to Calculate Sets of Prime Numbers
    /// Producer/consumer model for sharing resources
    /// the producers generate the numbers to be tested for prime
    /// consumers will read the numbers and check them for prime
    /// </summary>
	class Program
	{
        #region Fields
        // This Dictionary will contain a set of lists of the found primes. 
        // Each consumer thread will work with its own list.When all threads 
        // are done, these lists will be sorted and combined.
        static Dictionary<int, List<ulong>> m_Primes = new Dictionary<int, List<ulong>>();
        // Holds all of the numbers produced by the producer threads
        static ConcurrentQueue<ulong> m_Numbers = new ConcurrentQueue<ulong>();
        // Use this event to signal the consumers when the producers have finished their work
        static ManualResetEvent m_ProducersComplete = new ManualResetEvent(false);
        // used to prevent multiple threads from accessing the Console at the same time
        static object m_ConsoleLock = new object();
        // timer to periodically update the number of items left to process in the Console window
